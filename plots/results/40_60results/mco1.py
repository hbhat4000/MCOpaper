...
# This new statetimetally function is applications where we know exactly how much time is spent in each state of the time series.
def statetimetally1(ts,times,N):
    stally = np.zeros(N)
...
def ctmcG1(n, forcePos = False):
    ...
    x += np.repeat(np.arange(nb)*n*(n-1), 2*n*(n-1))
    if forcePos:
        ws = np.concatenate([eye, -seye, -eye, -seye, -eye, 0*eye, 0*eye, -eye, -eye]) 
        y = np.concatenate([y, np.arange(n*(n-1))])
        x = np.concatenate([x, nb*n*(n-1) + np.repeat(np.arange(n),n-1)])
        G = cvxopt.spmatrix(ws, x, y, ((nb*n*(n-1)) + n, 2*n*(n-1)))
    else:
...
def ctmcH(n, M, factor, forcePos = False):
    M_new = mShape(M, n)
    l = n*(n-1)
    myzeros = np.zeros(l)
...
def fixCTMC(M, w,factor, forcePos = False):
    n = len(w)
    G = ctmcG(n, forcePos)
    A = ctmcA(n, w)
    b = ctmcB(w, M)
    h = ctmcH(n, M, factor, forcePos)
    c = createC(n)
...
def test_naiveDTMC(ts, ts_test, wantST = True):
    ...
    if wantST:
        ...
        avgSTerrors = np.mean(STerrors)
        return train_errLT, test_errLT, STerrors, avgSTerrors
    else:
        return train_errLT, test_errLT
...
def test_fixedDTMC(ts, ts_test, wantST = True):
    ...
    if wantST:
        ...
        avgSTerrors = np.mean(STerrors)
        return trainLTerr, testLTerr, preds,STerrors, avgSTerrors, constrviol
    else:
        return trainLTerr, testLTerr, constrviol
...
def test_naiveCTMC(ts, tseq, ts_test, tseq_test, wantST = True):
    ...
    if wantST:
        ...
        ave_STerr = np.mean(STerrors)
        return LTerror_training, LTerror_test, STerrors, ave_STerr
    else:
...
def test_fixedCTMC(ts, tseq, ts_test, tseq_test, wantST = True):
    ...
    if wantST:
        ...
        ave_STerr = np.mean(STerrors)
        return LTerror_training, LTerror_test, STerrors, ave_STerr, constrviol, pivec
    else:
        return LTerror_training, LTerror_test, constrviol, pivec
